...
var xingzhi = {
    ...
    addNavBackgroundInit: function() {
        ...
        0 != (e - t > 0 ? e : t) && (document.getElementById("page-header").classList.add("nav-fixed"), document.getElementById("page-header").classList.add("nav-visible"), $("#cookies-window").hide())
    },
    ...
    addFriendLinksInFooter: function() {
        fetch("/zhxingzhi/friendlink.json").then((e => e.json())).then((e => {
            var t = [],
                o = -1;
            for (const n of e) {
                const e = n.link_list;
    ...
    addTag: function() {
        document.querySelector(".xingzhi-tag-new") && $(".xingzhi-tag-new").append('<sup class="xingzhi-tag xingzhi-tag-new-view">N</sup>'),
        document.querySelector(".xingzhi-tag-hot") && $(".xingzhi-tag-hot").append('<sup class="xingzhi-tag xingzhi-tag-hot-view">H</sup>')
    },
    ...
    downloadImage: function(e, t) {
        rm.hideRightMenu(),
        0 == rm.downloadimging ? (rm.downloadimging = !0, btf.snackbarShow("正在下载中，请稍后", !1, 1e4), setTimeout((function() {
            let o = new Image;
    ...
    changeThemeColor: function(e) {
        null !== document.querySelector('meta[name="theme-color"]') && (document.querySelector('meta[name="theme-color"]').setAttribute("content", e), document.querySelector('meta[name="apple-mobile-web-app-status-bar-style"]').setAttribute("content", e))
    },
    initThemeColor: function() {
        const e = window.scrollY || document.documentElement.scrollTop;
        if (xingzhi.is_Post()) {
            if (e > 0) {
                let e = getComputedStyle(document.documentElement).getPropertyValue("--xingzhi-card-bg");
                xingzhi.changeThemeColor(e)
            } else if (0 === e) {
                let e = getComputedStyle(document.documentElement).getPropertyValue("--xingzhi-main");
                xingzhi.changeThemeColor(e)
            }
        } else if (e > 0) {
            let e = getComputedStyle(document.documentElement).getPropertyValue("--xingzhi-card-bg");
            ...
            let e = getComputedStyle(document.documentElement).getPropertyValue("--xingzhi-background");
            xingzhi.changeThemeColor(e)
        }
    },
    jumpTo: function(e) {
    ...
    keyboardToggle: function() {
        xingzhi_keyboard ? (xingzhi_keyboard = !1, document.querySelector("#consoleKeyboard").classList.remove("on"), localStorage.setItem("keyboardToggle", "false")) : (xingzhi_keyboard = !0, document.querySelector("#consoleKeyboard").classList.add("on"), localStorage.setItem("keyboardToggle", "true"))
    },
    scrollTo: function(e) {
